...
const DEPLOYMENT_STATE_FILE = path.join(
  process.cwd(),
...
async function switchTraffic() {
  ...
  const inactivePort =
    inactiveEnv === "blue" ? state.bluePort : state.greenPort;
  ...
  state.activeEnvironment = inactiveEnv;
  ...
  console.log(
    `  Old environment: ${state.activeEnvironment === "blue" ? "green" : "blue"}`,
  );
...
function showStatus() {
  ...
  console.log(
    `  Port: ${state.activeEnvironment === "blue" ? state.bluePort : state.greenPort}`,
  );
  console.log(
    `  Version: ${state.activeEnvironment === "blue" ? state.blueVersion : state.greenVersion}`,
  );
  console.log(
...
async function simulateFullDeployment() {
  ...
  console.log(
    `Step 2: Deploy to ${inactiveEnv.toUpperCase()} (inactive environment)`,
  );
  console.log(`────────────────────────────────────`);
  ...
  console.log(`────────────────────────────────────`);
  console.log(
    `  ${state.activeEnvironment === "blue" ? "Green" : "Blue"} is now on standby`,
  );
